...
public class GaminActionServiceImpl implements GaminActionService {
    ...
    public List<Gaminrecord> getGaminrecordByUserId(int id,int start,int size){
        List<Gaminrecord> list=null;
        Gaminrecord gaminrecord=new Gaminrecord();
        gaminrecord.setUser_id(id);
        list=gaminrecordDAO.selectGaminrecord(gaminrecord,start,size);
        return list;
    }
    ...
    public int getUseridFromGaminid(int id){
        Gaminrecord gaminrecord=new Gaminrecord();
        gaminrecord.setGamin_id(id);
        List<Gaminrecord> gaminrecords=gaminrecordDAO.selectGaminrecord(gaminrecord,0,1);
        if(gaminrecords!=null){
            return gaminrecords.get(0).getUser_id();
        }
        return -1;
    }

    public int getGaminrecordMaxPage(int userid){
        return gaminrecordDAO.selectCountFromId(userid);
    }

    public int deletegaminfromid(int gaminrecordid,int gaminmessid){
        if(gaminrecordid>0 && gaminmessid>0){
            int flag1=gaminrecordDAO.deleteGaminrecord(gaminrecordid);
            int flag2=gaminmessDAO.deleteGaminmess(gaminmessid);
            if(flag1==flag2){
                return flag1;
            }
        }
