...
public class LostActionServiceImpl implements LostActionService {
    ...
    public int getUseridFromLostid(int id){
        Lostrecord lostrecord=new Lostrecord();
        lostrecord.setLost_id(id);
        List<Lostrecord> lostrecords=lostrecordDAO.selectLostrecord(lostrecord,0,1);
        if(lostrecords!=null){
            return lostrecords.get(0).getUser_id();
        }
        return -1;
    }

    public int deletelostfromid(int lostrecordid,int lostmessid){
        if(lostrecordid>0 && lostmessid>0){
            int flag1=lostrecordDAO.deleteLostrecord(lostrecordid);
            int flag2=lostmessDAO.deleteLostmess(lostmessid);
            if(flag1==flag2){
                return flag1;
            }
        }
